...
- **Tier 4:** Automated Playtesting + Fun Metrics + Asset Generation Bridge (COMPLETE ✅, TESTED ✅ — 62/62 pass, 3 bugs fixed)
...
### ESM `require()` in generation-backends.ts (FIXED — Tier 4)

- **File:** `src/utils/generation-backends.ts`
- **Issue:** `createMinimalPng()` used `const zlib = require('zlib')` which is CommonJS. The project uses ESNext modules, so `require()` is undefined at runtime.
- **Fix applied:** Replaced with `import { deflateSync } from 'zlib'` at module level.

### Playtest Recorder GDScript Compatibility (FIXED — Tier 4)

- **File:** `src/utils/playtest-recorder-gen.ts`
- **Issue:** Generated recorder script had three Godot 4.6 incompatibilities:
  1. Custom `round(value, decimals)` function shadowed built-in `round()` — Godot parser rejected 2-arg calls before seeing the custom definition
  2. `NOTIFICATION_WM_GO_BACK_REQUESTED` constant doesn't exist in Godot 4.6
  3. `var dx := arr[0] - arr[1]` — type inference fails on Variant subtraction
- **Fix applied:** Renamed custom round to `_rnd()`, removed invalid constant, added explicit `float()` casts with type annotations.
- **Note:** Any generated GDScript must be tested against the target Godot version. Godot's `round()` only takes 1 argument — use `roundf(value * factor) / factor` for decimal rounding.

### Windows Process Kill vs. Graceful Shutdown (FIXED — Tier 4)

- **File:** `src/tools/playtest.ts`, `src/utils/playtest-recorder-gen.ts`
- **Issue:** `stop_playtest_recording` called `process.kill()` which on Windows invokes `TerminateProcess()` — an immediate unconditional kill with no cleanup signals. The recorder's `_notification(NOTIFICATION_WM_CLOSE_REQUEST)` and `tree_exiting` signal never fire, so session data is never written.
- **Fix applied:** Implemented stop-file protocol:
  1. Stop handler creates `.mcp_playtest/_stop_<sessionId>` sentinel file
  2. Recorder checks for sentinel file every frame in `_process()`
  3. When found, recorder writes results and calls `get_tree().quit()`
  4. Stop handler polls for output JSON (250ms intervals, 5s max), then force-kills if needed
  5. Added `_results_written` guard to prevent double-writes from multiple exit paths

### Random/Stress Bot Input Recording (documented caveat — Tier 4)

- **Tools:** `run_automated_playtest` with `bot_type: "random"` or `"stress"`
- **Behavior:** These bots use `Input.action_press()`/`Input.action_release()` which are synthetic input injections that bypass Godot's `_input(event)` pipeline. As a result, `input_count` is always 0 for these bot types. The waypoint bot uses `Input.parse_input_event()` which does trigger `_input()` and produces recordable events. Responsiveness metrics require recorded inputs and are therefore only meaningful for waypoint bots and manual playtest sessions.
